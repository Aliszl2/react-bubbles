...
Tokens are random unique strings which are issued by the server and can be stored by the front end in local storage and be presented back to the server so they remember a user and can allow or deny (rendering) access to certain areas/components based on whether they are recognised and have logged in.
...
A web server is a software programme  that controls how web users access or  retrieve files, including dealing with web addresses.
